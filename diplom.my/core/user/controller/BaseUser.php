...
abstract class BaseUser extends \core\base\controller\BaseController
{
    ...
    protected function img($img = '', $tag = false){
        ...
        if(!$img && is_dir($_SERVER['DOCUMENT_ROOT'] . PATH . UPLOAD_DIR . DEFAULT_IMGAGE_DIRECTORY)){

            $dir = scandir($_SERVER['DOCUMENT_ROOT'] . PATH . UPLOAD_DIR . DEFAULT_IMGAGE_DIRECTORY);

            $imgArr = preg_grep('/' . $this->getController() . '\./i', $dir) ?: preg_grep('/default\./i', $dir);

            $imgArr && $img = DEFAULT_IMGAGE_DIRECTORY . '/' . array_shift($imgArr);
